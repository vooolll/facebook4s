...
class DomainParsing(asyncRequest: AsyncRequest) extends FailFastCirceSupport with LazyLogging {
  ...
  def asDomainResult[A](url: URLBuilder)(
    implicit domainContext: DomainParsingContext[A]
  ): Future[Either[FacebookError, A]] = {
    domainByUrl(url)
  }
  ...
  private[this] def domainByUrl[A](url: URLBuilder)(
    ...
    requestContext match {
      ...
      case context: PostRequestContext =>
        responseAsDomainResult(asyncRequest.post(url, context.params))
    }
  }

  private[this] def responseAsDomainResult[A](
  ...
  private[this] def valueOrException[A](result: Either[FacebookError, A]): A =
    result match {
      case Right(value) => value
      case Left(error)  => throw new RuntimeException(error.message)
    }

  private[this] def responseEntityResult(
    response: HttpResponse
  ): Either[HttpEntity, HttpEntity] = {
  ...
  private[this] def parseResult[A](
    ...
    httpEntityResult match {
      case Right(httpEntity) =>
        parse(httpEntity)(decoder, appResources) map (a => a.asRight)
      case Left(httpEntity) =>
        parse(httpEntity)(decodeError, appResources) map (b => b.asLeft)
    }
  }
}

object DomainParsing {
  def apply(asyncRequest: AsyncRequest): DomainParsing =
    new DomainParsing(asyncRequest)
