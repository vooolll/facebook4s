...
object FacebookDecoders {
  implicit val decodeTokenType: Decoder[AppAccessToken] =
    decodeString.map(AppAccessToken)
  implicit val decodeTokenValue: Decoder[TokenValue] =
    decodeString.map(TokenValue)
  implicit val decodePostId: Decoder[FacebookPostId] =
    decodeString.map(FacebookPostId)
  ...
  implicit val decodeProfileId: Decoder[FacebookProfileId] =
    decodeString.map(FacebookProfileId)

  implicit val decodeZoneOffset: Decoder[ZoneOffset] =
    decodeInt.map(ZoneOffset.ofHours)

  implicit val decodeUserAccessToken: Decoder[FacebookAccessToken] =
    new Decoder[FacebookAccessToken] {
      override def apply(c: HCursor) =
        ...
        } yield FacebookAccessToken(
          tokenValue,
          UserAccessToken(tokenType, expiresIn.seconds)
        )
    }
  ...
  implicit val decodeAppAccessToken: Decoder[FacebookAccessToken] =
    new Decoder[FacebookAccessToken] {
      override def apply(c: HCursor) =
        for {
          tokenValue <- c.get[TokenValue]("access_token")
  ...
  implicit val decodeError: Decoder[FacebookError] =
    new Decoder[FacebookError] {
      override def apply(c: HCursor) =
        for {
  ...
  implicit val decodeInstant: Decoder[Instant] = decodeString.emap { str =>
    Either
      .catchNonFatal(Instant.from(dateFormat.parse(str)))
      .leftMap(t => "Instant")
  }

  implicit val decodeUserId: Decoder[FacebookUserId] =
    decodeString.map(FacebookUserId)
  implicit val decodeAppId: Decoder[FacebookAppId] =
    decodeString.map(FacebookAppId)
  implicit val decodeClientId: Decoder[FacebookClientId] =
    decodeString.map(FacebookClientId.apply)

  implicit val decodeUserPicture: Decoder[FacebookUserPicture] =
    Decoder.forProduct4("height", "is_silhouette", "url", "width")(
      FacebookUserPicture
    )

  implicit val decodeTown: Decoder[FacebookTown] =
    Decoder.forProduct2("id", "name")(FacebookTown)
  ...
  implicit val decodeUser: Decoder[FacebookUser] = new Decoder[FacebookUser] {
    override def apply(c: HCursor) =
      for {
        ...
        picture <- c
          .downField("picture")
          .get[Option[FacebookUserPicture]]("data")
  ...
  implicit val decodeApplication: Decoder[FacebookApplication] =
    new Decoder[FacebookApplication] {
      override def apply(c: HCursor) =
        for {
    ...
    override def apply(c: HCursor) =
      for {
        ...
        attachmetns <- c
          ...
          .getOrElse[List[FacebookAttachment]]("data")(Nil)
      } yield FacebookPost(
        id,
        name,
        message,
  ...
  implicit val decodeLikesSummary: Decoder[FacebookLikesSummary] =
    new Decoder[FacebookLikesSummary] {
      override def apply(c: HCursor) =
        for {
  ...
  implicit val decodeLikes: Decoder[FacebookLikes] =
    new Decoder[FacebookLikes] {
      override def apply(c: HCursor) =
        for {
  ...
  implicit val decodeMediaObjectId: Decoder[FacebookMediaObjectId] =
    decodeString.map(FacebookMediaObjectId)

  implicit val decodeMediaObject: Decoder[FacebookMediaObject] =
    Decoder.forProduct2("id", "created_time")(FacebookMediaObject)

  implicit val decodeCommentId: Decoder[FacebookCommentId] =
    decodeString.map(FacebookCommentId)

  implicit val decodeComment: Decoder[FacebookComment] =
    new Decoder[FacebookComment] {
      override def apply(c: HCursor) =
        for {
  ...
  implicit val decodeCommentSummary: Decoder[FacebookCommentSummary] =
    new Decoder[FacebookCommentSummary] {
      override def apply(c: HCursor) =
        for {
  ...
  implicit val decodeComments: Decoder[FacebookComments] =
    new Decoder[FacebookComments] {
      override def apply(c: HCursor) =
        for {
  ...
  implicit val decodeImage: Decoder[FacebookImage] =
    new Decoder[FacebookImage] {
      override def apply(c: HCursor) =
        for {
  ...
  implicit val decodeAlbumId: Decoder[FacebookAlbumId] =
    decodeString.map(FacebookAlbumId)

  implicit val decodeAlbum: Decoder[FacebookAlbum] =
    new Decoder[FacebookAlbum] {
      override def apply(c: HCursor) =
        for {
  ...
  implicit val decodeAlbums: Decoder[FacebookAlbums] =
    new Decoder[FacebookAlbums] {
      override def apply(c: HCursor) =
        for {
  ...
  implicit val decodePhotoId: Decoder[FacebookPhotoId] =
    decodeString.map(FacebookPhotoId)

  implicit val decodePhoto: Decoder[FacebookPhoto] =
    new Decoder[FacebookPhoto] {
      override def apply(c: HCursor) =
        for {
  ...
  implicit val decodeAttachmentTargetId: Decoder[FacebookAttachmentId] =
    Decoder.decodeString.map(FacebookAttachmentId)
  ...
  implicit val decodeAttachmentType: Decoder[AttachmentType] =
    decodeString.map {
  ...
  implicit val decodeAttachment: Decoder[FacebookAttachment] =
    new Decoder[FacebookAttachment] {
      override def apply(c: HCursor) =
        for {
          attachment <- c
            .downField("media")
            .get[Option[FacebookImageSource]]("image")
  ...
  implicit val decodeFriendsSummary: Decoder[FacebookFriendsSummary] =
    Decoder.forProduct1("total_count")(FacebookFriendsSummary)

  implicit val decodeFriends: Decoder[FacebookFriends] =
    new Decoder[FacebookFriends] {
      override def apply(c: HCursor) =
        for {
