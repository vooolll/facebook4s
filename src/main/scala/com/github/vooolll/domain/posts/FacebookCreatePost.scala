...
case class FacebookCreatePost(
  ...
  def toMap()(implicit accessToken: FacebookAccessToken) = {
    val mandatoryPart =
      Map("message" -> message, "access_token" -> accessToken.tokenValue.value)

    val optionalPart = {
      link.map("link" -> _.toString) ++ name.map("name" -> _) ++ description
        .map("description" -> _)
    } ++ {
      caption.map("caption" -> _) ++ picture.map("picture" -> _.toString) ++ published
        .map("published" -> _.toString)
    } ++ {
      callToAction.map("call_to_action" -> _.toString()) ++ thumbnail.map(
