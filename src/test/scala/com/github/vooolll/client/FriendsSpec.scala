...
class FriendsSpec extends FacebookClientSupport {
  ...
  "Graph Api" should {
    "return user friends" in { c =>
      c.friends(FacebookUserId("me"))
        .map(_.withoutQueryParams shouldBe facebookFriends)
    }

    "return user friends result" in { c =>
      c.friendsResult(FacebookUserId("me"))
        .map(_.map(_.withoutQueryParams) shouldBe facebookFriends.asRight)
    }
  }
}
