...
object TokenApp extends App {
  ...
  val authUriToGetCode = facebookClient.buildAuthUrl(List(Posts))
  val authUriToGetToken = facebookClient.buildAuthUrl(List(Posts), responseType = FacebookToken)

  println(authUriToGetCode)
  println(authUriToGetToken)

  val shortLivingToken = Properties.envOrNone("FACEBOOK_TEST_USER_ACCESS_TOKEN") getOrElse config.getString("facebook.testUserAccessToken")
  ...
  longLivedToken onComplete {
    case Success(token) =>  println(token) // long lived token you can use it on server side, or send it to client
