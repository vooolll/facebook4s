...
val examplVersionLine = exampleBuidSbtFile
  .find(_.contains("facebook4sVersion ="))
  .getOrElse(
    throw new RuntimeException("Failed to find facebook4s example version")
  )
val targetVersionLine = targetBuildSbtFile
  .find(_.contains("version :="))
  .getOrElse(
    throw new RuntimeException("Failed to find facebook4s target version")
  )

val targetVersion = targetVersionLine.dropWhile(_ != '=')

assert(
  examplVersionLine.contains(targetVersion),
